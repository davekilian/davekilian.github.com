...
going on. This way of debugging is slow and unproductive. Most people find it
hard to make sense of per-pixel color output.

Since debugging is so hard, it'd be nice not to need to debug your shaders in
the first place, if it can be helped ;). This post focuses on preventing the
most common shader programming mistake: mixing up your transformations.

## Symptoms

You can get all sorts of bizarre behavior, depending on how your shader's
...
You might say vector spaces are to vectors as units are to numbers. If you
have an equation made of vector additions, subtractions, dot products, or
what-have-you, the math only works out if the all vectors occupy the same
vector space. 

If you need to convert vectors to a different vector space, you can use a
_transformation_. Then, once everything's in the same vector space, the math
works out and your shaders are (hopefully) bug-free. Hooray!

This is easier said than done, but it's quite doable if you understand how
these transformations work and how they tie into the graphics pipeline.

## How Transformations Work

You've probably seen vector transforms before. They allow you to scale, rotate
and translate the objects in your scene.
...
Before we talk about how vector spaces and vector transforms play into the
graphics pipeline, we should start by naming the spaces that appear over and
over within the pipeline. 

### Object Space

Scenes are composed of primitives (like spheres and cylinders), models
(triangle meshes), or some combination of the two. An "object" is simply one of
these primitives or models. 

Object space is a convenient coordinate system for defining an object in your
scene. Consider a cylinder as our example object. We might define the object
space for that cylinder as follows:

* The origin is at the cylinder's center
* The $Y$ axis is aligned with the cylinder's axis
* The $XZ$ plane cuts the cylinder in half
* One unit along the $X$, $Y$ or $Z$ axis corresponds to the radius of the
  cylinder.

These choices were arbitrary, as we could have defined the same cylinder in a
different coordinate system. This choice is just convenient for both the code 
that creates the cylinder's vertices as well as the code that uses them.
...
The 'camera' is an abstract concept representing the viewer's position and
orientation. Eye space is simply the camera's object space. It is defined so
that the camera is at the origin, looking down the negative $Z$ axis, with $Y$
pointing up.

It turns out representing the scene in eye space makes it easier to render
(more on that later). Once objects have been transformed from object space to
world space, we typically end up transforming them from world space to eye
space.
...
The $Z$ axis corresponds to depth buffering values. A vertex with $Z = 0$ in clip
space is sitting on the near plane, and one with $Z = -1$ lies on the far plane
...
Screen space is the only 2D space in this list. It is defined so that
...
$w$ and $h$ are respecitvely the width and height of your display in pixels.
...
TODO talk about how vertex processing works, how transforms are combined into a
single matrix, how this is a big win from a Big-O standpoint

Also, mention there's no notion of a camera.

Then segue into needing to compose transformations through all these vector
spaces.
