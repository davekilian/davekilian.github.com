...
Well, it certainly works if every term uses the same unit:
$5 m + 2 m = 7 m$.
...
mismatch: convert some of the terms to a different unit of
...
You might say vector spaces are to vectors as units are to numbers. Like with
units, if you have an equation made of vector additions, subtractions, dot 
products, or what-have-you, the math only works out if the all vectors occupy 
...
conversions. Math involving vectors only 'works'
after you've transformed all the vectors to the same vector space. Once you've
done that, your shaders will be (hopefully) bug-free!

This is easier said than done, but it's manageable if you understand how
...
$M_{AB}$ is the identity matrix. If objects in $A$-space are half as large as
objects in $B$-space, $M_{AB}$ is just a scale matrix. 
