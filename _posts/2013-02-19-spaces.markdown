...
The relationship between the GPU and CPU makes it hard to debug shaders.
Without printlines or a proper debugger, oftentimes shader programmers
resort to using the fragment shader's output color as a way of testing
their assumptions. This leads to slow and unproductive debugging sessions.

Thus it's better to not need to debug your shaders in the first place, if it
can be helped :). One of the most common shader bugs is the misuse of
transformations. Unfrotunately, messing up your transformations can make your
shader act weird in all sorts of ways:
...
The easiest way to avoid transformation-related bugs is to carefully define
what transformations should be applied to which vectors, by convention.

This post overviews what transformations are, how they're used in conventional
rasterized 3D rendering, and specifically how they are used in OpenGL today.
The author will assume that the reader has some experience with 3D graphics
(e.g. using lighting models and transforming objects), as well as a little
...
Does $5 + 2 = 7$? This equation is not necessarily true when we take units into
account:

* If every term uses the same units of measurement, the equation holds:
  $5 m + 2 m = 7 m$
...
Like units, vector spaces define how values may be used together. You might say
units are for scalar numbers what vector spaces are for vectors. Since your
scene is made of vectors, vector spaces tend to come in handy!

Why are vector spaces handy? Before we noticed that it makes no sense to
directly add two numbers that have different units. Likewise, it makes no sense
to operate on two vectors (via addition, dot product, what have you) unless 
they're both in the same vector space.

If that's true, we need a way to go from one vector space to another, like we
did with units. The tool for moving between vector spaces
is called the *transformation*. You've probably seen these before; they allow
you to scale, rotate and translate the objects in your scene.

Mathematically, each transformation is represented by a transformation matrix.
If a matrix transforms from $A$-space to $B$-space, and a vector is defined in
$A$ space, then multiplying the two together produces the same vector in
$B$-space.

You can build complex transformations by daisy-chaining the simpler ones.
Transformation matrices are combined via matrix multiplication:

* If $M\_{AB}$ transforms from $A$-space to $B$-space
* And $M\_{BC}$ transforms from $B$-space to $C$-space
* Then $M\_{BC} \times M\_{AB}$ transforms from $A$-space to $B$-space to 
  $C$-space; in other words, it transforms from $A$-space to $C$-space

This process ends up looking a lot like dimensional analysis.
