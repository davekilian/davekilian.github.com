...
The resulting setup, of an ARIES write-ahead log sitting in front of a b-tree written in-place on disk, was the de facto state of the art in databases throughout most of the 1990s. Although both technologies are database-specific, file systems developed analogous structures: journals instead of WALs, and inode trees instead of b-trees. Modern file systems have started to close this loop; btrfs is based on b-trees and has a WAL-like journal, for example.

## Performance Characteristics of a B-Tree/ARIES Strategy

Now that we understand the basic structure of an ARIES log sitting in front of a b-tree which tracks SQL rows or key-value pairs, let's ask: what are the performance characteristics of this system? When you push this system to its limits, which part of the system will fall over first?
