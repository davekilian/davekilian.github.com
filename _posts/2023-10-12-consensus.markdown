...
Groups of programs get stuck on decisions too. Usually it’s because there’s some data that two servers are trying to update at the same time: if each server starts doing its update not knowing the other server is also starting an update, they can end up disagreeing over what the final state of that data should be. Like with human consensus, the servers now need to reach agreement on what the updated state should be, and stick to it so they both show users the same data. And like before, the goal is to make a quick decision and move on; users might be waiting on those updates! So both servers can accept either update so long as it’s guaranteed both accepted the same update. That means one server successfully does its update and the other one fails, but this failure might be okay: the server that failed to do its update can probably retry, or barring that report an actionable error to the user.

We call algorithms that make these kinds of decisions  *consensus* algorithms, since they make unanimous group decisions like people do. The kinds of conflicts that can be resolved with consensus algorithms are called consensus problems.

That’s the gist, but it might be helpful to see how servers can get into disagreements in practice:

## Example Use Cases

Let’s take a look at a few examples of situations where conflicts between servers can arise &mdash; examples where a consensus algorithm is needed:
