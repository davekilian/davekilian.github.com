...
When distributed systems programmers use the term “consensus” to describe their code, they’re noticing that sometimes programs need to make group decisions, just like us. 

Think of a time you and a group of other people were stuck on a decision. Maybe you all wanted to go to a restaurant but couldn’t decide where to go, or you all wanted to go see a movie but couldn’t decide which one. If you’ve ever accepted a group decision even though you didn’t get exactly what you wanted, you already understand how consensus works! (If not, maybe you should rethink some life choices!) The word “consensus” basically means “agreement,” but it usually also implies urgency and finality: everybody wants to make an agreement and stick to it, so the group can move on (e.g. go see that movie). People might have individual preferences, but won’t jeopardize consensus by pushing their own views too hard, because having a decision is more important than getting what they think is the best decision. And once consensus is reached, nobody’s going to be willing to reopen past disagreements: the point of getting to consensus is to end the disagreement and be done with it.

Groups of programs get stuck on decisions too. Usually it’s because there’s some data that two servers are trying to update at the same time: if each server starts doing its update not knowing the other server is also starting an update, they can end up disagreeing over what the final state of that data should be. Like with human consensus, the servers now need to reach agreement on what the updated state should be, and stick to it so that all users see the same data. And like before, the goal is to make a quick decision and move on; those users might be waiting on those updates! So even though each server prefers its own update, both can accept either update as long as they have the guarantee that both accepted the same one. That means the other server’s update failed, but that’s okay: it can probably retry, or barring that the user might be able to do something about the failure.

We call algorithms that solve this problem  *consensus* algorithms, since they implement group decision making between servers a lot like how groups of people need to make decisions too.

That’s the gist anyways. But it might be helpful to see a few of these disagreements in practice:
