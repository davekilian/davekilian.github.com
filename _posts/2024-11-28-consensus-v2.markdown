...
It used to be that any serious education in distributed systems involved learning the Paxos consensus algorithm, and that was bad because Paxos was (and still is) notoriously difficult to understand. Even the experts didn’t understand the Paxos paper when it was first published (in fact, so few people understood the paper right away, Paxos in general ended up [languishing in obscurity for the better part of a decade](https://www.microsoft.com/en-us/research/publication/part-time-parliament/)). Needless to say, if the experts can’t make heads or tails of Paxos, students are really going to struggle. But for a long time, Paxos was the only known solution to consensus, and consensus is an important problem, so subjecting students to Paxos was a cruel necessity.

Nowadays there is a second consensus algorithm, *Raft*, which accomplishes the same basic things as Paxos and works fundamentally in a similar way, while also being much eaier to understand. Should one subject themselves to Paxos now that Raft exists?

Well, I wouldn’t be writing this if I didn’t think there were some things about Paxos that are worth your time :)

It comes down to the design philosophies of the two algorithms. Raft is a large, complex algorithm, and the authors address that complexity using separation of concerns, breaking the overall algorithm into smaller sub problems that can be solved independently, then composing these solutions to form the full Raft algorithm. This factoring helps makes the algorithm easier to digest, implement and test. If you intend to implement a consensus algorithm, Raft is probably the paper for you.

Paxos is more mathematical, focusing more on proofs of possibility and correctness than implementation. Paxos works by designing a “core” algorithm which solves a simplified problem once, on top of which we then heap layers and extensions to get ever more mileage out of that simple core algorithm, eventually landing on a more complete solution to a larger problem. This strategy, of repeatedly saying “if I have a solution to X, I can use that to solve Y” is a useful way to write existence proofs, but actually implementing an algorithm this way leads to an undebuggable spaghetti mess, and nobody wants that. However, the net net here is that Paxos contains within it a small, simple and insightful core algorithm; this algorithm arguably exists in Raft too, but it has been factored into pieces in tbd name if making the full algorithm simpler.
