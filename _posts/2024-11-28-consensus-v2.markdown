...
In the bad old days, any serious education in distributed systems involved learning the Paxos consensus algorithm, and that was a bad thing because Paxos was (and still is) notoriously difficult to understand. The original Paxos paper didn’t even make sense to expert-level researchers at the original time of its publishing, to the point of Paxos [languishing in obscurity for the better part of a decade ](https://www.microsoft.com/en-us/research/publication/part-time-parliament/) because too few of the people who read it understood it well enough to see what had been accomplished. For students of the field, the hill to climb is even steeper. But for a time, Paxos was the only known solution to consensus, and consensus is an important problem, so subjecting students to Paxos was a cruel necessity.

Nowadays there is a second consensus algorithm, *Raft*, which accomplishes the same basic things as Paxos and works the same way at a basic level, while also being much eaier to understand. Should one subject themselves to Paxos now that Raft exists?

Raft is all the more appealing if you intend to implement any of the things you are reading in these papers. Despite its length and complexity, the Paxos paper is still only a sketch; to implement it, you will need to fill in many of the blanks yourself. Others have done this, and many found it so difficult that they ended up publishing *their own* papers describing the implementation problems they ran into and what they recommend for solving it. If you need to implement consensus, Raft is your friend, because you get a complete algorithm, all in one paper, stated as simply and specifically as its authors could. The paper's authors provide a reference implementation, and other people have manged to implement and deploy Raft without feeling the need to publish additional papers about the experience.

So yeah, Raft has a lot of things going for it. But I wouldn't be here writing this if I didn’t think Paxos had some things going for it too :)

Raft has certainly made this understandability situation better, but in my reading, Raft is more of a recipe than an insight &mdash; it's a feat of factoring more than a completely new way to look at the consensus problem. That is a useful thing, but when I study an algorithm, what I usually want to get out of it is to find a crux: what is the tricky problem at the heart of the thing, and what’s the trick we chose for solving it? For example, binary search uses the idea of eliminating half the remaining search space in each step; Dijkstra’s algorithm is about exploring the shortest remaining segment you know of so far; what’s that idea for a consensus algorithm like Raft or Paxos? Neither paper exactly answers that, but Paxos gets much closer to the crux than Raft.

Raft and Paxos are both big algorithms, but they’re internally factored pretty differently: Raft uses separation of concerns to split the problem into more manageable pieces and solve them independently; Paxos instead uses a “little” algorithm that solves a simplified version of the problem once, and then adds a bunch of extensions on top of the little algorithm to get more mileage out of it and eventually solve a bigger problem. This “little” algorithm inside of Paxos, which the paper calls the *Synod* algorithm in keeping with its parliamentary metaphor, is small, insightful, and gets to the crux of the problem. Whether or not you ever design a full consensus system (few people will ever need to), I still think the synod algorithm is worth learning if you want to have a strong foundation in distributed algorithms.

Leslie Lamport, who invented Paxos, [once described Paxos's synod algorithm](https://lamport.azurewebsites.net/pubs/paxos-simple.pdf) as "among the simplest and most obvious of distributed algorithms," and claimed that it "follows unavoidably from the properties we want it to satisfy." Even so, I would not go so far as to call it simple or obvious :) There many consensus algorithms simpler and more obvious than the synod algorithm &mdash; but none of them work, and they are all broken in subtle and non-obvious ways. To see the synod algorithm as simple and obvious, one must first find that every simpler and more obvious algorithm does not actually solve the problem in the first place. Once you see the pattern in why these algorithms don't work, the synod algorithm really does start to seem like a straightforward and clear solution to the conensus problem &mdash; and you will also have seen the crux of the problem.
