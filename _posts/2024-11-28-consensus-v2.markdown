...
Distributed consensus algorithms are a critical piece of modern computing infrastructure. Also, few people really understand them. The very first one, *Paxos*, was at first met with met with indifference, because people thought it was some kind of elaborate joke. (It probably did not help the presenter was dressed up in an Indiana Jones costume.) For the next 25 years, Paxos remained the only viable algorithm for distributed consensus. Then, about a decade ago, a second viable contender appeared: *Raft*. The paper that introduced Raft was called *In Search of an Understandable Consensus Algorithm* because evidently, in the ensuing 25 years, nobody had been able to figure out Paxos.

Well, it’s not exactly fair to say “nobody” could figure it out. Not everyone agrees Paxos is hard to understand. Its inventor for one claims that Paxos is "among the simplest and most obvious of distributed algorithms," and that it "follows unavoidably from the properties we want it to satisfy." Does Dr. Lamport believe this because Dr. Lamport is a wizard who inhabits some other astral plane? Could be, but he has a point: the hard part about Paxos isn't Paxos, it's understanding the problem space well enough to see why you need to solve consensus exactly the way Paxos does it. Even Raft doesn’t really differ from Paxos at a fundamental level: it is better factored and the mechanics are easier to explain, but the fundamental approach to solving consensus is basically no different, because there isn't really a fundamentally simpler way to do it.

Maybe that means there’s a better way to learn about consensus algorithms: by learning the problem space first. Once we see that intuitive algorithms fail and we figure out what is causing them to fail, and we know what we have to change to get around that, Paxos really will seem like the simplest possible solution. 

To get there. we‘ll start by looking at the problem of consensus, to see why it matters and also understand the mechanics of what a consensus algorithm needs to do. Then we're going to try out a couple "obvious" algorithms and see that they end up not working in non-obvious ways. In trying to figure out they don’t work, we'll end up with the *FLP result*, which uses formal mathematical language to explain what is wrong with the obvious approaches we tried so far. Once we eliminate approaches FLP says doesn’t work and then try again, we'll stumble right into Paxos &mdash; specifically, the core "synod" algorithm that implements the consensus part of Paxos.
