...
There are some legitimate arguments for skipping it. For one, Paxos is notoriously difficult to understand &mdash; so much so that it [languished in relative obscurity for the better part of a decade](https://www.microsoft.com/en-us/research/publication/part-time-parliament/) before it started to be taken seriously in the mainstream. Plus, Raft exists. The Raft paper was called, *In Search of an Understandable Consensus Algorithm*, because Raft’s big accomplishment is that it solves the same problem as Paxos without being as unintuitive and explanation-defying as Paxos.
...
You see, the original Paxos paper tried to do too much with too little; if you try to implement what you find in the paper, you’ll realize much of what you’re trying to build is given only as a rough sketch, and then you, like everyone else who has implemented Paxos before you, will spend years reinventing the wheel (and then optionally publishing your own “here’s how we did Paxos” follow-up paper, also like others before you). If you need to implement a consensus system, Raft is your friend, because the paper just gives you a complete picture, all in one place, as simply and specifically as its authors could muster.

But unlike Raft, Paxos is layered around a small, simple, insightful algorithm, called the *Synod* algorithm (in keeping with the paper’s “parliamentary” theme). Whether or not you ever design a full consensus system (few people will ever need to), I still think the synod algorithm is worth your time.

Leslie Lamport, who invented Paxos, [once described Paxos's synod algorithm](https://lamport.azurewebsites.net/pubs/paxos-simple.pdf) as "among the simplest and most obvious of distributed algorithms," and claimed that it "follows unavoidably from the properties we want it to satisfy." I would say that is half true. The synod algorithm is certainly among the simplest and most obvious of consensus algorithms, but that does not make it simple or obvious. Consensus is just hard :) There are much simpler and much more obvious attempts at consensus algorithms, but alas they are all broken, in subtle and unintuitive ways. To see the synod algorithm as simple and obvious, you must first see every simpler and more obvious algorithm does not actually solve the problem.
