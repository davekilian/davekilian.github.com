...
Distributed consensus algorithms are known primarily for two things: playing a small but critical role in distributed / cloud computing infrastructure, and for being obscure. You can work on cloud software all your life and never interact with a consensus algorithm at all, let alone build one, or run one; yet everything you build in the cloud relies on a consensus algorithm in one way or another, because without consensus, your cloud running your code itself would not work. 

Or, so they say. You see, consensus algorithms are obscure in more way than one: not only are they hard to learn, it's also unclear what role they play, exactly. What do consensus algorithms do? What goes wrong if we don't have them? How can it be that nothing works without consensus, yet nobody really knows about them or has to interact with them? In this deep dive, we will aim to find out.
