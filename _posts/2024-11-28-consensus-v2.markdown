...
There are some legitimate arguments for skipping it. For one, Paxos is notoriously difficult to understand &mdash; so much so that it [languished in relative obscurity for the better part of a decade](https://www.microsoft.com/en-us/research/publication/part-time-parliament/) before it started to be taken seriously in the mainstream. Plus, Raft exists. The Raft paper was called, *In Search of an Understandable Consensus Algorithm*, because Raft’s big accomplishment is that it solves the same problem as Paxos without being as intuitive and explanation-defying as Paxos.

These are reasonable points, but obviously we wouldn’t be here if I thought that was the end of the story :)

Raft has certainly made this understandability situation better, but in my opinion Raft is more of a recipe than an insight. When I study an algorithm, what I really want to get out of it usually is to find a crux: what is the tricky problem at the heart of the algorithm, and what’s the interesting trick for solving it? For example, binary search uses the idea of eliminating half the remaining search space in one step; Dijkstra’s algorithm is about exploring the shortest remaining segment you know of so far; what’s that idea for a consensus algorithm? The Raft paper doesn’t really answer that, and in my opinion the best way to go about finding the crux of consensus is to revisit our old friend Paxos.

You see, the original Paxos paper tried to do too much with too little; if you try to implement what you find in the paper, you’ll realize much of what you’re trying to build is given only as a rough sketch, and then you, like everyone else who has implemented Paxos before you, will spend years reinventing the wheel (and then possibly, like many others before you, publish your own “here’s how we did Paxos” follow-up paper). Raft is much better in this regard, because it just gives you a complete picture, all in one place, as simply and specifically as it can. 

But unlike Raft, Paxos is layered around a smaller, simpler, more insightful algorithm. This heart of Paxos is called the *Synod* algorithm, in keeping with the paper’s goofy parliamentary metaphor. Whether or not you ever design a full consensus system (few people will ever need to), I still think the synod algorithm is worth your time.

Leslie Lamport, who invented Paxos, [once described Paxos's synod algorithm](https://lamport.azurewebsites.net/pubs/paxos-simple.pdf) as "among the simplest and most obvious of distributed algorithms," and claimed that it "follows unavoidably from the properties we want it to satisfy." That is undeniably true, but for the synod algorithm to seem obvious you must first do a lot of footwork: you need to really understand the problem we are solving, and you need to see the pattern in why all the 'intuitive' ways to solve consensus actually don’t work. So maybe that's the best way to learn about consensus: let's figure out what exactly a consensus algorithm should do, try the immediately obvious approaches to solving it, find they are all broken, and then get stuck. Then, in getting ourselves un-stuck, we'll have to find the pattern in why our attempts so far haven't worked, and that should lead us "unavoidably" into the synod algorithm &mdash; one of the simplest and most obvious consensus algorithms.

However, our ultimate goal will not be to build a full consensus system; implementing exactly what we find in the Raft paper is a more expedient way of doing that. Instead, we want to get to the crux of consensus.
