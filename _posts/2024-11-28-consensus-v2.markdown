...
I think it’s safe to say most programmers *have* heard of consensus, but the only things programmers end up needing to know in general about consensus are (1) distributed systems people say it’s super important and fundamental and (2) consensus is hard. Few people claim to deeply understand how consensus algorithms work, and those who do often have trouble explaining their thoughts to other people, so it remains obscure.

I've always found it intriguing that consensus algorithms can be so important, yet so obscure. The solution to the paradox, I think, is that there aren't really all that many implementations of consensus deployed and running out in the wild &mdash; but the ones that do exist touch almost everything you do online. Maybe the obscurity of consensus is in part driven by how centralized modern computing infrastructure has become.

But the situation with consensus and understandability is pretty bad &mdash; and always has been. The first consensus algorithm, *Paxos*, was introduced to the world in 1989, and at the time it was initially met with a sort of [indifferent confusion](https://www.microsoft.com/en-us/research/publication/part-time-parliament/). It took another 9 years for the algorithm to gain enough grassroots popularity to be taken seriously. As the world moved online and distributed systems became the way new software was built, Paxos found its way into the bottom layer of those new systems, but at the same time, it also grew notorious for being beyond the comprehension of mere mortals. In fact, when a second viable consensus algorithm, *Raft*, finally came along well over a decade later, the paper was called *In Search of an Understandable Consensus Algorithm*, because the accomplishment was not in creating a new consensus algorithm, but rather in being able to explain how a consensus algorithm works at all!

Raft has certainly made this understandability situation better, but in my opinion it doesn’t really get into the heart: what is the kernel of consensus, the main technique being used to solve a tricky problem? For example, binary search uses the idea of eliminating half the remaining search space in one step; Dijkstra’d algorithm is about traversing the shortest remaining segment you know of so far; what’s that idea for a consensus algorithm? For that, I think we have to turn back to Paxos &mdash; good ol’ notorious-for-being-confusing Paxos :-).

That’s because there is a kernel at the core of Paxos is a little protocol called the *synod algorithm*, which is small, well-specified, insightful, and can be embedded into other systems and algorithms in a way that Raft and the full ‘multi-Paxos’ algorithm cannot. Whether or not you ever design a full consensus system (as we said, few people will ever need to), I still think the synod algorithm is worth your time.
...
However, our ultimate goal will not be to build a full consensus algorithm; instead, we want to get to the kernel of an idea at the heart of consensus. As a preview, the kernel of the synod algorithm is to “work around” the CAP theorem by using “1-way” decisions, so that even if you don’t know exactly what happened on a node you’ve lost contact with, there is still one outcome you can safely assume. The point of this whole article is for that to sound sensible, instead of being gobbledegook.
