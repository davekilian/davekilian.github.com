...
There was a time when Paxos was a “must learn” for people who are serious about distributed systems, but I think that day has passed. Paxos is notoriously hard to learn, to the point of [languishing in obscurity for the better part of a decade](https://www.microsoft.com/en-us/research/publication/part-time-parliament/) because, at the time of its publishing, there were too few people who actually understood the paper for Paxos to go mainstream. Plus, Raft exists. The Raft paper was called, *In Search of an Understandable Consensus Algorithm*, because Raft’s accomplishment is to solve the same problem as Paxos without being as unintuitive and explanation-defying as Paxos. Why subject yourself to the pain that is learning Paxos if there’s a less painful option for doing the same thing?

The contrast between Raft and Paxos is doubly stark if you actually intend to implement any of the things you read in these papers. Despite its length and complexity, the Paxos paper is still just a sketch; to implement it, you will need to fill in many of the blanks yourself. Others have done this before you, and many found it so difficult that they ended up publishing *their own* papers describing the problems they ran into just implementing Paxos and what they recommend for solving it. Needless to say, every implementation of Paxos is its own unique and special flower. If you need to implement a consensus system, Raft is your friend, because the paper just gives you a complete picture, all in one place, as simply and specifically as its authors could muster.

These are all valid points in favor of skipping Paxos and learning Raft instead, but obviously we wouldn’t be here if I thought that’s actually what you should do :)

Raft has certainly made this understandability situation better, but in my eyes Raft is more of a recipe than an insight. When I study an algorithm, what I usually want to get out of it is to find a crux: what is the tricky problem at the heart of the algorithm, and what’s the interesting trick for solving it? For example, binary search uses the idea of eliminating half the remaining search space in one step; Dijkstra’s algorithm is about exploring the shortest remaining segment you know of so far; what’s that idea for a consensus algorithm? The Raft paper doesn’t really answer that, and in my opinion Paxos is your better bet. 

This is because Paxos is layered around a small, simple, insightful algorithm, called the *Synod* algorithm (in keeping with the paper’s “parliamentary” theme). Whether or not you ever design a full consensus system (few people will ever need to), I still think the synod algorithm is worth your time.

Leslie Lamport, who invented Paxos, [once described Paxos's synod algorithm](https://lamport.azurewebsites.net/pubs/paxos-simple.pdf) as "among the simplest and most obvious of distributed algorithms," and claimed that it "follows unavoidably from the properties we want it to satisfy." Even so, that does not make it simple or obvious. Consensus is hard :) The only reason the synod algorithm can be the simplest and most obvious algorithm that works is because there are many simpler, more obvious algorithms that are actually broken, often in subtle, unintuitive ways. To see the synod algorithm as simple and obvious, one must first find that every simpler and more obvious algorithm does not actually solve the problem. Once you see the crux of the problem and how the synod algorithm approaches it, it actually does end up looking quite simple. 
