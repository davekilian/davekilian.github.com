...
Distributed consensus algorithms are known primarily for two things: playing a small but critical role in online computing infrastructure, and for being obscure &mdash; arcane, even. You can work on cloud software all your life and never interact with a consensus algorithm at all, let alone build one, or run one; yet everything you build in the cloud fundamentally on a consensus algorithm one way or another, because without consensus, your cloud running your code itself would not work. 

Or, so they tell us. Consensus algorithms are doubly obscure: not only are they hard to understand, it's hard to say exactly what role they play in the first place. What do consensus algorithms do? What goes wrong if we don't have one? How can it be that nothing works without consensus, yet nobody really knows about them or has to interact with them? In this deep dive, we will aim to find out.
