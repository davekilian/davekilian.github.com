...
The original Paxos paper arguably tried to do too much with too little; if you try to implement what you find in the paper, you’ll realize much of what you’re trying to build is given only as a rough sketch, and then you, like everyone else who has implemented Paxos before you, will spend years filling in those blanks, probably reinventing the wheel and possibly even publishing your own “here’s how we did Paxos” follow-up paper &mdash; all like others have done before you. If you need to implement a consensus system, Raft is your friend, because the paper just gives you a complete picture, all in one place, as simply and specifically as its authors could muster.

But compared to Raft, Paxos gets closer to the crux of the problem. Paxos is layered around a small, simple, insightful algorithm, called the *Synod* algorithm (in keeping with the paper’s “parliamentary” theme). Whether or not you ever design a full consensus system (few people will ever need to), I still think the synod algorithm is worth your time.

Leslie Lamport, who invented Paxos, [once described Paxos's synod algorithm](https://lamport.azurewebsites.net/pubs/paxos-simple.pdf) as "among the simplest and most obvious of distributed algorithms," and claimed that it "follows unavoidably from the properties we want it to satisfy." I would say I half agree with this. The synod algorithm is certainly among the simplest and most obvious of consensus algorithms, but that does not make it simple or obvious. Consensus is just hard :) The only reason the synod algorithm can be the simplest and most obvious algorithm that works is because there are many simpler, more obvious algorithms that turn out to be broken, often in subtle, unintuitive ways. To see the synod algorithm as simple and obvious, one must first find that every simpler and more obvious algorithm does not actually solve the problem.
