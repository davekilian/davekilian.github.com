...
There was a time when Paxos was a “must learn” for anybody serious about distributed systems. But those were bad times! Paxos is notorious for being difficult to understand, and not just for beginners: Paxos spent the better part of a decade [languishing in obscurity](https://www.microsoft.com/en-us/research/publication/part-time-parliament/), unable to reach the mainstram because so few of the researchers and editors who had read the paper understod it well enough to see what had been accomplished. For anyone with less experience, the hill to climb is even steeper. But for a time, Paxos was the only known solution to consensus, and consensus is an important problem, so subjecting students to Paxos was a cruel necessity.

Nowadays there is a second consensus algorithm, *Raft*, which accomplishes the same basic things as Paxos while being much eaier to understand. Why subject yourself to the pain that is learning Paxos if there’s another algorithm which accomplishes the same thing but is way easier to learn?

In fact, Raft is all the more appealing if you intend to implement any of the things you are reading in these papers. Despite its length and complexity, the Paxos paper is still only a sketch; to implement it, you will need to fill in many of the blanks yourself. Others have done this, and many found it so difficult that they ended up publishing *their own* papers describing the implementation problems they ran into and what they recommend for solving it. If you need to implement consensus, Raft is your friend, because you get a complete algorithm, all in one paper, stated as simply and specifically as its authors could. The paper's authors provide a reference implementation, and other people have manged to implement and deploy Raft without feeling the need to publish additional papers about the experience.
