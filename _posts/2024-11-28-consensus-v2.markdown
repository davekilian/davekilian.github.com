...
There was a time when Paxos was a “must learn” for anybody serious about distributed systems. But those were bad times: Paxos is notoriously confusing. Immediately following its invention, Paxos spent the better part of a decade [languishing in obscurity](https://www.microsoft.com/en-us/research/publication/part-time-parliament/) because there were so few people in the entire field who understood the paper or realized what had been accomplished. Even people who know distributed systems well find that it takes a lot of time and energy to decipher Paxos; you can imagine how it usually goes for novices. But it used to be that Paxos was the only known solution to consensus, and consensus is an important problem, so subjecting students to Paxos was a cruel necessity. 

Until it wasn't. Nowadays there is a second consensus algorithm, *Raft*, which accomplishes the same basic things as Paxos without being so difficult to understand. Why subject yourself to the pain that is learning Paxos if there’s another algorithm which accomplishes the same thing but is way easier to learn?

If you intend to implement any of the things you read in these papers, the contrast between Raft and Paxos is even more stark. Despite its length and complexity, the Paxos paper is still only a sketch; to implement it, you will need to fill in many of the blanks yourself. Others have done that, and many found it so difficult that they ended up publishing *their own* papers describing the problems they ran into and what they recommend for solving it. If you need to implement consensus, Raft is your friend, because you get a complete algorithm, all in one paper, stated as simply and specifically as its authors could muster.

So yeah, there are a lot of arguments in favor of learning Raft and skipping Paxos. But wouldn’t be here writing this if I thought that was the whole story :)

Raft has certainly made this understandability situation better, but in my eyes Raft is more of a recipe than an insight &mdash; a feat of factoring more than a completely new way to do things. I respect that, but when I study an algorithm, what I usually want to get out of it is to find a crux: what is the tricky problem at the heart of the algorithm, and what’s the neat trick for solving it? For example, binary search uses the idea of eliminating half the remaining search space in one step; Dijkstra’s algorithm is about exploring the shortest remaining segment you know of so far; what’s that idea for a consensus algorithm? The Raft paper doesn’t really answer that, and in my opinion Paxos is your better bet.

This is because Paxos is layered around a small, simple, insightful algorithm, which the paper calls the *Synod* algorithm in keeping with its “parliamentary” theme. Whether or not you ever design a full consensus system (few people will ever need to), I still think the synod algorithm is worth your time.

Leslie Lamport, who invented Paxos, [once described Paxos's synod algorithm](https://lamport.azurewebsites.net/pubs/paxos-simple.pdf) as "among the simplest and most obvious of distributed algorithms," and claimed that it "follows unavoidably from the properties we want it to satisfy." Even so, it's still so simple or obvious :) There many consensus algorithms simpler and more obvious than the synod algorithm, but they are all broken in subtle and non-obvious ways. To see the synod algorithm as simple and obvious, one must first find that every simpler and more obvious algorithm does not actually solve the problem. Only when you see the pattern in why these algorithms don't work, does the synod algorithm start to seem like a straightforward and clear solution to the problem.
