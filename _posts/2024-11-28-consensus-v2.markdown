...
The first consensus algorithm, *Paxos*, was introduced to the world in 1989. It was met with a sort of [indifferent confusion](https://www.microsoft.com/en-us/research/publication/part-time-parliament/), and only gradually gained enough grassroots popularity over the next 9 years to be taken seriously. As the world moved online and distributed systems became the way new software was built, Paxos found its way into the bottom layer of those new systems, but at the same time, it also grew notorious for being beyond the comprehension of mere mortals. In fact, when a second viable consensus algorithm, *Raft*, finally came along over a decade later, the paper was called *In Search of an Understandable Consensus Algorithm*, because the accomplishment they were claiming was not in creating a new consensus algorithm, but rather in being able to explain how a consensus algorithm works at all!

Raft has certainly made this understandability situation better, but in my opinion Raft is more of a recipe than an insight. When I study an algorithm, I like to try and find the crux: what is the tricky problem we need to solve, and what’s the trick for solving it? For example, binary search uses the idea of eliminating half the remaining search space in one step; Dijkstra’s algorithm is about exploring the shortest remaining segment you know of so far; what’s that idea for a consensus algorithm? For that, I think we have to turn back to Paxos &mdash; good ol’ notorious-for-being-confusing Paxos :-).

That’s because there is a kernel at the core of Paxos is a little protocol called the *synod algorithm*, which is small, well-specified, and insightful. The synod algorithm is the crux of Paxos. Whether or not you ever design a full consensus system (few people will ever need to), I still think the synod algorithm is worth your time.

Leslie Lamport, who invented Paxos, [once described Paxos's synod algorithm](https://lamport.azurewebsites.net/pubs/paxos-simple.pdf) as "among the simplest and most obvious of distributed algorithms," and claimed that it "follows unavoidably from the properties we want it to satisfy." That is undeniably true, but the synod algorithm only starts to seem obvious once you have done a lot of footwork: you need to really understanding the problem we are solving, and you need to see the pattern in why all the 'more intuitive' ways to solve consensus actually don’t work. So maybe that's the best way to learn about consensus: let's figure out what exactly a consensus algorithm should do, try the immediately obvious approaches to solving it, find they are all broken and then get stuck. Then, in getting ourselves un-stuck, we'll have to find the pattern in why our attempts so far haven't worked, and that should lead us "unavoidably" into the synod algorithm &mdash; the simplest and most obvious consensus algorithm.

However, our ultimate goal will not be to build a full consensus algorithm; instead, we want to get to the crux of consensus. I would roughly describe that crux as ‘one-way’ voting to avoid getting into CAP-like situations; my goal for this article is for that to sound sensible, and not like gobbledygook. 
