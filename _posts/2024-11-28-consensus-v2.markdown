...
In the bad old days, any serious education in distributed systems involved learning the Paxos consensus algorithm, and that was a bad thing because Paxos was (and still is) notoriously difficult to understand. The original Paxos paper didn’t even make sense to expert-level researchers at the original time of its publishing, and because so few people understood the paper right away, Paxos in general ended up [languishing in obscurity for the better part of a decade](https://www.microsoft.com/en-us/research/publication/part-time-parliament/). And that’s the experts; for students the climb is all the steeper. But for a time, Paxos was the only known solution to consensus, and consensus is an important problem, so subjecting students to Paxos was a cruel necessity.
