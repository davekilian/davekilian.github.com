...
Well, Raft has a lot of things going for it, but I wouldn’t be writing this if I didn’t think there were some things about Paxos that are worth your time too :)

It all comes down to the design philosophies of the two algorithms. Raft is a paper for implementers: the Raft algorithm is provided completely and plainly in a single paper, in recipe form. Raft is a large, complex algorithm, but the authors use separation of concerns to break the overall algorithm into smaller sub problems that can be solved independently, with their solutions composing to form the full Raft algorithm. This factoring makes the algorithm much easy to digest, implement and test. If you intend to implement a consensus algorithm, Raft is probably the paper for you.

Paxos is more of a mathematical thing. Paxos works by designing a “core” algorithm which solves a simplified problem once, and then heaping on layers and extensions to get ever more mileage out of that simple core algorithm, eventually landing on a more complete solution to a larger problem. This strategy, of always saying “if I have a solution to X, I can also solve Y by extending my solution to X” is a useful way to write existence proofs, but actually implementing an algorithm this way leads to an undebuggable spaghetti mess, and nobody wants that. However, the net net here is that Paxos contains within it a small, simple and insightful core algorithm; this algorithm arguably exists in Raft too, but it has been factored into pieces in tbd name if making the full algorithm simpler.

When I study an algorithm, what I usually want to get out of it is to find a crux: what is the tricky problem at the heart of the thing, and what’s the trick we chose for solving it? For example, binary search uses the idea of eliminating half the remaining search space in each step; Dijkstra’s algorithm is about exploring the shortest remaining segment you know of so far; what’s that idea for a consensus algorithm like Raft or Paxos? The answer that question certainly lies somewhere in Paxos’s synod algorithm. 
