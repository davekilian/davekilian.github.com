...
Distributed consensus algorithms are a critical piece of modern computing infrastructure. Also, nobody really understands them. The very first one, *Paxos*, was initially met with met with indifference; people thought it was some kind of elaborate joke. (It probably did not help the presenter was dressed up in an Indiana Jones costume.) For the next 25 years, Paxos remained the only viable algorithm for distributed consensus. About a decade ago, a second viable contender appeared: *Raft*. The paper that introduced Raft was called *In Search of an Understandable Consensus Algorithm*; evidently, in the ensuing 25 years, nobody could figure out Paxos.

Now, not everyone agrees Paxos is indecipherable. Its inventor for one claims that Paxos is "among the simplest and most obvious of distributed algorithms," and that it "follows unavoidably from the properties we want it to satisfy." Although it's fun to quip Dr. Lamport believes this because Dr. Lamport is a wizard who inhabits some other astral plane, the fact remains that he has a point: the hard part about Paxos isn't Paxos, it's understanding the problem space and the solution space well enough to see why you need to solve consensus exactly the way Paxos does it. In fact, Raft isn't really that different from Paxos: it is better factored and the mechanics are a little easier to explain, but the fundamental approach to solving consensus is basically the same as Paxos, because there isn't really a fundamentally simpler way of solving consensus.
